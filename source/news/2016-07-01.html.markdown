...
=======
<<<<<<< HEAD
At the time of writing this, there’s a weird issue with rubygems.org index where `bundle install` won’t work in your project because it doesn’t find latest version of dry-logic. For now please **install dry-validation manually** and then do `bundle install`, so:
=======
At the time of writing this, there’s a weird issue with the rubygems.org index, and `bundle install` won’t work in your project because it doesn’t find latest version of dry-logic. For now please **install dry-validation manually** and then do `bundle install`, so:
>>>>>>> origin/ann-post-dry-v
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
We have refined existing macros and added a couple of new ones. This allows new types of rule compositions that were not possible before. First of all the new `filled` macro (which replaces `required`) and the `maybe` macro now support blocks too, and both are based on a new, simpler macro called `value`. The `key` macro has been renamed to `required` which makes it easier to understand the difference between defining required or optional keys and rules for their values.
=======
<<<<<<< HEAD
We have refined existing macros and added a couple of new ones to allow rule compositions which were not possible before.

The new `filled` (which replaces `required`) and `maybe` now support blocks too, and both are based on a simpler, new macro called `value`. The `key` macro has been renamed to `required` which makes it easier to understand the difference between defining required or optional keys and rules for their values.
=======
We have refined existing macros and added a couple of new ones. This allows new types of rule compositions that were not possible before. First of all the new `filled` macro (which replaces `required`) and the `maybe` macro now support blocks too, and both are based on a new, simpler macro called `value`. The `key` macro has been renamed to `required` which makes it easier to understand the difference between defining required or optional keys and rules for their values.
>>>>>>> origin/ann-post-dry-v
>>>>>>> Stashed changes
