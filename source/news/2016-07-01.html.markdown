...
At the time of writing this, there’s a weird issue with the rubygems.org index where `bundle install` won’t work in your project because it doesn’t find latest version of dry-logic. For now please **install dry-validation manually** and then do `bundle install`, so:
